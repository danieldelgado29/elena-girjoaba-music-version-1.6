(() => {
  ...
  function placeImageTextAt(clientX,clientY){const stage=$('#imageEditorStage'),paper=imageEditorPaper(),sr=stage.getBoundingClientRect(),scale=Math.max(.0001,imageEditorState.scale);const localX=(clientX-sr.left-imageEditorState.panX)/scale,localY=(clientY-sr.top-imageEditorState.panY)/scale,pw=Math.max(1,paper.offsetWidth),ph=Math.max(1,paper.offsetHeight),margin=4,x=Math.max(margin,Math.min(pw-margin,localX)),y=Math.max(margin,Math.min(ph-margin,localY));imageEditorState.textX=x/pw;imageEditorState.textY=y/ph;imageInlineText.style.left=`${imageEditorState.textX*100}%`;imageInlineText.style.top=`${imageEditorState.textY*100}%`;imageInlineText.style.width=`${Math.max(120,Math.min(pw-x-4,pw*.7))}px`;imageInlineText.style.height=`${Math.max(58,Math.min(ph-y-4,ph*.35))}px`;imageInlineText.dataset.anchorX=String(imageEditorState.textX);imageInlineText.dataset.anchorY=String(imageEditorState.textY);imageInlineText.hidden=false;syncInlineTextStyle();/* iPhone/Safari solo abre el teclado si focus ocurre dentro del gesto */try{imageInlineText.focus({preventScroll:true});}catch(_){imageInlineText.focus();}const end=imageInlineText.value.length;try{imageInlineText.setSelectionRange(end,end);}catch(_){}requestAnimationFrame(()=>{try{imageInlineText.focus({preventScroll:true});imageInlineText.setSelectionRange(end,end);}catch(_){}});}
