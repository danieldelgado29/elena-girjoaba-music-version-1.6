(() => {
  ...
  const imageEditorState={original:'',overlay:'',sources:[],tool:'pencil',size:8,pencilColor:'#d00000',textColor:'#d00000',drawMode:'free',eraserTarget:'annotations',drawing:false,last:null,path:[],undo:[],redo:[],textBold:false,textItalic:false,scale:1,panX:0,panY:0,pointers:new Map(),pinch:null,panning:null};
  ...
  function openImageEditor(songId,owner){
    ...
    Object.assign(imageEditorState,{tool:'pencil',size:8,pencilColor:'#d00000',textColor:'#d00000',drawMode:'free',eraserTarget:'annotations',drawing:false,textBold:false,textItalic:false,scale:1,panX:0,panY:0});imageInlineText.value='';imageInlineText.hidden=true;
  ...
  function syncInlineTextStyle(){imageInlineText.style.color=imageEditorState.textColor;imageInlineText.style.fontWeight=imageEditorState.textBold?'700':'400';imageInlineText.style.fontStyle=imageEditorState.textItalic?'italic':'normal';imageInlineText.style.fontSize=`${Math.max(24,imageEditorState.size*4)}px`;$('#imageBold').classList.toggle('is-active',imageEditorState.textBold);$('#imageItalic').classList.toggle('is-active',imageEditorState.textItalic);syncImageSwatches();}
  function activateImageText(){imageEditorState.tool='text';$('#imageTextTool').classList.add('is-active');$('#imageToolPencil').classList.remove('is-active');$('#imageToolEraser').classList.remove('is-active');imageInlineText.hidden=false;syncInlineTextStyle();requestAnimationFrame(()=>{imageInlineText.focus({preventScroll:true});const n=imageInlineText.value.length;imageInlineText.setSelectionRange(n,n);});}
  function commitImageText(){const text=imageInlineText.value;if(!text.trim()){imageInlineText.value='';imageInlineText.hidden=true;return;}const c=imageEditorCanvas(),ctx=imageEditorContext();const cssSize=Math.max(24,imageEditorState.size*4),scale=c.width/Math.max(1,c.getBoundingClientRect().width/imageEditorState.scale),fontSize=cssSize*scale;const pad=c.width*.05,maxWidth=c.width-pad*2,lineHeight=fontSize*1.25;ctx.save();ctx.globalCompositeOperation='source-over';ctx.fillStyle=imageEditorState.textColor;ctx.font=`${imageEditorState.textItalic?'italic ':''}${imageEditorState.textBold?'700':'400'} ${fontSize}px -apple-system, BlinkMacSystemFont, sans-serif`;ctx.textAlign='left';ctx.textBaseline='top';let y=c.height*.05;for(const paragraph of text.split(/\n/)){const words=paragraph.split(/\s+/);let line='';for(const word of words){const test=line?line+' '+word:word;if(ctx.measureText(test).width>maxWidth&&line){ctx.fillText(line,pad,y);y+=lineHeight;line=word;}else line=test;}if(line)ctx.fillText(line,pad,y);y+=lineHeight;}ctx.restore();imageInlineText.value='';imageInlineText.hidden=true;pushImageHistory();}
  $('#imageTextTool').addEventListener('click',activateImageText);
  let textHold=0;$('#imageTextTool').addEventListener('pointerdown',e=>{textHold=setTimeout(()=>positionPopover($('#imageTextOptions'),e.currentTarget),550);});['pointerup','pointercancel','pointerleave'].forEach(n=>$('#imageTextTool').addEventListener(n,()=>clearTimeout(textHold)));
  ...
  $$('[data-image-pencil-color]').forEach(b=>b.addEventListener('click',()=>{imageEditorState.pencilColor=b.dataset.imagePencilColor;$('#imagePencilOptions').hidden=true;syncImageSwatches();}));
  ...
  $('#imageToolEraser').addEventListener('click',()=>{if(!imageInlineText.hidden)commitImageText();imageEditorState.tool='eraser';$('#imageToolEraser').classList.add('is-active');$('#imageToolPencil').classList.remove('is-active');$('#imageTextTool').classList.remove('is-active');});
  ...
  $$('[data-image-eraser-size]').forEach(btn=>btn.addEventListener('click',()=>{imageEditorState.size=Number(btn.dataset.imageEraserSize);imageEditorState.tool='eraser';$('#imageDrawSize').value=String(imageEditorState.size);$('#imageEraserOptions').hidden=true;}));
  $('#imageDrawSize').addEventListener('change',e=>imageEditorState.size=Number(e.target.value));$('#imageDrawMode').addEventListener('change',e=>imageEditorState.drawMode=e.target.value);
  ...
  function strokeArrow(ctx,a,b,both=false){const ang=Math.atan2(b.y-a.y,b.x-a.x),len=Math.max(18,imageEditorState.size*3);const head=(p,angle)=>{ctx.moveTo(p.x,p.y);ctx.lineTo(p.x-len*Math.cos(angle-Math.PI/6),p.y-len*Math.sin(angle-Math.PI/6));ctx.moveTo(p.x,p.y);ctx.lineTo(p.x-len*Math.cos(angle+Math.PI/6),p.y-len*Math.sin(angle+Math.PI/6));};head(b,ang);if(both)head(a,ang+Math.PI);}
  imageEditorCanvas().addEventListener('pointerdown',e=>{if(imageEditorState.tool==='text'||e.pointerType==='touch'&&imageEditorState.pointers.size>0)return;e.preventDefault();imageEditorState.drawing=true;imageEditorState.last=imagePoint(e);imageEditorState.path=[imageEditorState.last];e.currentTarget.setPointerCapture(e.pointerId);});
  imageEditorCanvas().addEventListener('pointermove',e=>{if(!imageEditorState.drawing)return;e.preventDefault();const p=imagePoint(e),target=imageEditorState.tool==='eraser'&&imageEditorState.eraserTarget==='photo'?imageBaseCanvas():imageEditorCanvas(),ctx=target.getContext('2d');ctx.save();ctx.lineCap='round';ctx.lineJoin='round';ctx.lineWidth=imageEditorState.size;ctx.strokeStyle=imageEditorState.pencilColor;ctx.globalCompositeOperation=imageEditorState.tool==='eraser'?'destination-out':'source-over';ctx.beginPath();ctx.moveTo(imageEditorState.last.x,imageEditorState.last.y);ctx.lineTo(p.x,p.y);ctx.stroke();ctx.restore();imageEditorState.last=p;imageEditorState.path.push(p);});
  const finishImageDraw=()=>{if(!imageEditorState.drawing)return;imageEditorState.drawing=false;if(imageEditorState.tool==='pencil'&&imageEditorState.drawMode!=='free'&&imageEditorState.path.length>1){const ctx=imageEditorContext(),a=imageEditorState.path[0],b=imageEditorState.path.at(-1);ctx.save();ctx.strokeStyle=imageEditorState.pencilColor;ctx.lineWidth=imageEditorState.size;ctx.lineCap='round';ctx.beginPath();strokeArrow(ctx,a,b,imageEditorState.drawMode==='double-arrow');ctx.stroke();ctx.restore();}pushImageHistory();};imageEditorCanvas().addEventListener('pointerup',finishImageDraw);imageEditorCanvas().addEventListener('pointercancel',finishImageDraw);
  ...
  imageStage.addEventListener('pointerdown',e=>{imageEditorState.pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(imageEditorState.pointers.size===2){const [a,b]=[...imageEditorState.pointers.values()];imageEditorState.pinch={distance:Math.hypot(a.x-b.x,a.y-b.y),cx:(a.x+b.x)/2,cy:(a.y+b.y)/2};}else if(e.target===imageStage){imageEditorState.panning={id:e.pointerId,x:e.clientX,y:e.clientY,panX:imageEditorState.panX,panY:imageEditorState.panY};imageStage.setPointerCapture?.(e.pointerId);}},true);
  imageStage.addEventListener('pointermove',e=>{if(imageEditorState.pointers.has(e.pointerId))imageEditorState.pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(imageEditorState.pointers.size===2&&imageEditorState.pinch){const [a,b]=[...imageEditorState.pointers.values()],d=Math.hypot(a.x-b.x,a.y-b.y),cx=(a.x+b.x)/2,cy=(a.y+b.y)/2;zoomImageAt(cx,cy,d/imageEditorState.pinch.distance);imageEditorState.panX+=cx-imageEditorState.pinch.cx;imageEditorState.panY+=cy-imageEditorState.pinch.cy;imageEditorState.pinch={distance:d,cx,cy};applyImageTransform();e.preventDefault();}else if(imageEditorState.panning?.id===e.pointerId){imageEditorState.panX=imageEditorState.panning.panX+e.clientX-imageEditorState.panning.x;imageEditorState.panY=imageEditorState.panning.panY+e.clientY-imageEditorState.panning.y;applyImageTransform();e.preventDefault();}},true);
  const endImagePointer=e=>{imageEditorState.pointers.delete(e.pointerId);if(imageEditorState.pointers.size<2)imageEditorState.pinch=null;if(imageEditorState.panning?.id===e.pointerId)imageEditorState.panning=null;};imageStage.addEventListener('pointerup',endImagePointer,true);imageStage.addEventListener('pointercancel',endImagePointer,true);
  $('#saveImageEditorBtn').addEventListener('click',()=>{if(!imageInlineText.hidden)commitImageText();const song=state.songs.find(x=>x.id===activeImageSongId);if(!song)return;askConfirm('Guardar imagen',`Se actualizará la imagen de ${ownerLabel(activeImageOwner)} para “${song.titulo}”.`,()=>{const composite=imageEditorComposite(),overlay=imageEditorCanvas().toDataURL('image/png'),base=imageBaseCanvas().toDataURL('image/png');song[imageField(activeImageOwner)]={original:base,overlay,composite};const ci=state.customSongs.findIndex(x=>x.id===song.id);if(ci>=0)state.customSongs[ci]={...song};else state.songEdits[song.id]={...song};saveStateLocalOnly();syncRemoteState(true);rememberDialogState($('#imageEditorDialog'));toast('Guardado exitosamente');renderSongbookList();},'Guardar');});
